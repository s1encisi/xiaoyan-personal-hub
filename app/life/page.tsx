import { LifeDriftWall } from "../_components/effects/ai-effects";
...
import { JournalYearNav } from "../_components/journal";
import { journalEntries, journalPhotoCount } from "../_data/journal";
...
export default function LifePage() {
  return (
    <main id="main-content" className="life-home" tabIndex={-1}>
      ...
      <PortfolioSection id="daily-journal" code="BY YEAR" title="把日常，也认真留下" intro={`${journalEntries.length} 则记录、${journalPhotoCount} 幅图像。校园、旅行、演出和共同的兴趣，按年份保留当时的语气。`}><JournalYearNav /><p><a href="/life/journal">浏览完整日常图文档案 ↗</a></p><a className="gaming-entry" href="/life/gaming"><div><small>GAMING / MY OTHER WORLDS</small><h3>游戏足迹</h3><p>CS、王者荣耀、原神、鸣潮……<br />从竞技对局到角色与世界，翻开 14 张真实记录。</p></div><ArrowRight size={28} aria-hidden="true" /></a></PortfolioSection>
      <section className="section-shell life-drift-section" aria-labelledby="life-drift-title"><h2 id="life-drift-title">让照片，带回当时的风景</h2><LifeDriftWall items={journalEntries.filter(entry => entry.category === "travel" && entry.photos.length > 0).slice(0,8).map(entry => ({ image: entry.photos[0].small, title: entry.title, href: `/life/journal/${entry.year}#entry-${entry.slug}` }))} /></section>
