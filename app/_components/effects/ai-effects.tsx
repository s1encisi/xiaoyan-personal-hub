...
export function KnowledgeWheel({ chapters }: { chapters: NotionChapter[] }) {
  ...
  return <div ref={ref} className="knowledge-wheel" data-effect="Option Wheel" data-effect-active={active}><div className="knowledge-wheel__control">{active ? <SafeEffect fallback={fallback}><OptionWheel items={chapters.map(c => c.title)} defaultSelected={index} onChange={change} fontSize={1.8} activeColor="#e8f4ff" textColor="#a4bbcc" curve={0.45} tilt={4} blur={0.6} inset={24} soundVolume={0} /></SafeEffect> : fallback}</div><div className="knowledge-wheel__preview"><span>沿着一个主题继续</span><h2>{selected.title}</h2><p>{selected.summary}</p><a href={`#${selected.slug}`}>进入章节导引 <ArrowRight size={18} /></a></div></div>;
}
