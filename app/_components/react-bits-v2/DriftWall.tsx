"use client";
/* eslint-disable @next/next/no-img-element -- Local optimized images or decorative texture layers. */
// React Bits, David Haz. Adapted from c49d6978d2496660f0f0c5a3b3ca77a059566a93; see LICENSE.md.
import { CSSProperties, useCallback, useEffect, useLayoutEffect, useMemo, useRef, useState, useSyncExternalStore } from 'react';
...
const subscribeReducedMotion = (callback: () => void) => { const query = window.matchMedia('(prefers-reduced-motion: reduce)'); query.addEventListener('change', callback); return () => query.removeEventListener('change', callback); };
...
const DriftWall = ({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number, duplicate = false) => {
    const inner = (
    ...
    if (item.href) {
      return (
        <a key={id} href={item.href} tabIndex={duplicate ? -1 : 0} aria-hidden={duplicate || undefined} {...commonProps}>
          {inner}
        </a>
      );
    }
            ...
            <div className="drift-wall__col" key={`col-${c}`}>
              ...
              >
                {copies.map((_, copyIndex) =>
                  col.map((item, itemIndex) => renderTile(item, `${c}-${copyIndex}-${itemIndex}`, c, copyIndex > 0))
                )}
              </div>
            </div>
          );
